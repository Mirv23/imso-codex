from django.urls import path

from .views import (
    DashboardView,
    audit_log_list,
    admin_user_create,
    admin_user_detail,
    admin_user_list,
    account_change_password,
    blog_create,
    blog_detail,
    blog_list,
    bulk_delete_section,
    booking_create,
    booking_detail,
    booking_list,
    bookings_reset,
    site_settings_detail,
    upload_image,
    contact_detail,
    contact_list,
    chapter_create,
    chapter_detail,
    chapter_list,
    chapter_reorder,
    chapter_video_confirm,
    chapter_video_remove,
    chapter_video_upload_url,
    course_enrollment_create,
    course_enrollment_detail,
    course_enrollment_list,
    learner_create,
    learner_detail,
    learner_list,
    teacher_options,
    teacher_kyc_overview,
    student_overview,
    student_detail,
    student_export,
    course_create,
    course_detail,
    course_list,
    course_overview,
    course_students,
    dashboard_charts,
    dashboard_summary,
    enrollment_detail,
    enrollment_list,
    export_csv,
    gei_create,
    gei_detail,
    gei_list,
    gei_overview,
    member_create,
    member_detail,
    member_list,
    member_overview,
    member_profile,
    order_detail,
    order_list,
    order_payments,
    product_create,
    product_detail,
    product_list,
    notification_check,
    notification_clear,
    notification_list,
    notification_read,
    notification_read_all,
    payment_create,
    payment_detail,
    payment_list,
    provider_create,
    provider_detail,
    provider_list,
    provider_overview,
    provider_test_connection,
    core_value_create,
    core_value_detail,
    core_value_list,
    siteimage_create,
    siteimage_detail,
    siteimage_list,
    sitetext_create,
    sitetext_detail,
    sitetext_list,
    process_step_create,
    process_step_detail,
    process_step_list,
    testimonial_create,
    testimonial_detail,
    testimonial_list,
)

app_name = "adminpanel"

urlpatterns = [
    path("", DashboardView.as_view(), name="dashboard"),
    path("api/summary/", dashboard_summary, name="summary"),
    path("api/audit-log/", audit_log_list, name="audit-log"),
    path("api/v1/summary/", dashboard_summary, name="summary-v1"),
    path("api/charts/", dashboard_charts, name="charts"),
    path("api/members/", member_list, name="member-list"),
    path("api/members/overview/", member_overview, name="member-overview"),
    path("api/members/<int:pk>/profile/", member_profile, name="member-profile"),
    path("api/v1/members/", member_list, name="member-list-v1"),
    path("api/members/create/", member_create, name="member-create"),
    path("api/v1/members/create/", member_create, name="member-create-v1"),
    path("api/members/<int:pk>/", member_detail, name="member-detail"),
    path("api/v1/members/<int:pk>/", member_detail, name="member-detail-v1"),
    path("api/courses/", course_list, name="course-list"),
    path("api/courses/overview/", course_overview, name="course-overview"),
    path("api/courses/<int:pk>/students/", course_students, name="course-students"),
    path("api/courses/<int:course_pk>/chapters/", chapter_list, name="chapter-list"),
    path("api/courses/<int:course_pk>/chapters/create/", chapter_create, name="chapter-create"),
    path("api/courses/<int:course_pk>/chapters/reorder/", chapter_reorder, name="chapter-reorder"),
    path("api/chapters/<int:pk>/", chapter_detail, name="chapter-detail"),
    path("api/chapters/<int:pk>/video-upload-url/", chapter_video_upload_url, name="chapter-video-url"),
    path("api/chapters/<int:pk>/video-confirm/", chapter_video_confirm, name="chapter-video-confirm"),
    path("api/chapters/<int:pk>/video-remove/", chapter_video_remove, name="chapter-video-remove"),
    path("api/learners/", learner_list, name="learner-list"),
    path("api/learners/create/", learner_create, name="learner-create"),
    path("api/learners/<int:pk>/", learner_detail, name="learner-detail"),
    path("api/teacher-options/", teacher_options, name="teacher-options"),
    path("api/learners/kyc-overview/", teacher_kyc_overview, name="teacher-kyc-overview"),
    path("api/course-enrollments/", course_enrollment_list, name="course-enrollment-list"),
    path("api/course-enrollments/create/", course_enrollment_create, name="course-enrollment-create"),
    path("api/students/overview/", student_overview, name="student-overview"),
    path("api/students/export/", student_export, name="student-export"),
    path("api/students/<int:pk>/", student_detail, name="student-detail"),
    path("api/course-enrollments/<int:pk>/", course_enrollment_detail, name="course-enrollment-detail"),
    path("api/v1/courses/", course_list, name="course-list-v1"),
    path("api/courses/create/", course_create, name="course-create"),
    path("api/v1/courses/create/", course_create, name="course-create-v1"),
    path("api/courses/<int:pk>/", course_detail, name="course-detail"),
    path("api/v1/courses/<int:pk>/", course_detail, name="course-detail-v1"),
    path("api/bookings/", booking_list, name="booking-list"),
    path("api/bookings/create/", booking_create, name="booking-create"),
    path("api/bookings/reset/", bookings_reset, name="booking-reset"),
    path("api/v1/bookings/", booking_list, name="booking-list-v1"),
    path("api/bookings/<int:pk>/", booking_detail, name="booking-detail"),
    path("api/v1/bookings/<int:pk>/", booking_detail, name="booking-detail-v1"),
    path("api/payments/", payment_list, name="payment-list"),
    path("api/v1/payments/", payment_list, name="payment-list-v1"),
    path("api/payments/create/", payment_create, name="payment-create"),
    path("api/v1/payments/create/", payment_create, name="payment-create-v1"),
    path("api/payments/<int:pk>/", payment_detail, name="payment-detail"),
    path("api/v1/payments/<int:pk>/", payment_detail, name="payment-detail-v1"),
    path("api/contacts/", contact_list, name="contact-list"),
    path("api/v1/contacts/", contact_list, name="contact-list-v1"),
    path("api/contacts/<int:pk>/", contact_detail, name="contact-detail"),
    path("api/v1/contacts/<int:pk>/", contact_detail, name="contact-detail-v1"),
    path("api/geis/", gei_list, name="gei-list"),
    path("api/geis/overview/", gei_overview, name="gei-overview"),
    path("api/v1/geis/", gei_list, name="gei-list-v1"),
    path("api/geis/create/", gei_create, name="gei-create"),
    path("api/v1/geis/create/", gei_create, name="gei-create-v1"),
    path("api/geis/<int:pk>/", gei_detail, name="gei-detail"),
    path("api/v1/geis/<int:pk>/", gei_detail, name="gei-detail-v1"),
    path("api/providers/", provider_list, name="provider-list"),
    path("api/providers/overview/", provider_overview, name="provider-overview"),
    path("api/providers/<int:pk>/test/", provider_test_connection, name="provider-test"),
    path("api/v1/providers/", provider_list, name="provider-list-v1"),
    path("api/providers/create/", provider_create, name="provider-create"),
    path("api/v1/providers/create/", provider_create, name="provider-create-v1"),
    path("api/providers/<int:pk>/", provider_detail, name="provider-detail"),
    path("api/v1/providers/<int:pk>/", provider_detail, name="provider-detail-v1"),
    path("api/enrollments/", enrollment_list, name="enrollment-list"),
    path("api/v1/enrollments/", enrollment_list, name="enrollment-list-v1"),
    path("api/enrollments/<int:pk>/", enrollment_detail, name="enrollment-detail"),
    path("api/v1/enrollments/<int:pk>/", enrollment_detail, name="enrollment-detail-v1"),
    path("api/notifications/", notification_list, name="notification-list"),
    path("api/v1/notifications/", notification_list, name="notification-list-v1"),
    path("api/notifications/check/", notification_check, name="notification-check"),
    path("api/v1/notifications/check/", notification_check, name="notification-check-v1"),
    path("api/notifications/<int:pk>/read/", notification_read, name="notification-read"),
    path("api/v1/notifications/<int:pk>/read/", notification_read, name="notification-read-v1"),
    path("api/notifications/read-all/", notification_read_all, name="notification-read-all"),
    path("api/v1/notifications/read-all/", notification_read_all, name="notification-read-all-v1"),
    path("api/notifications/clear/", notification_clear, name="notification-clear"),
    path("api/testimonials/", testimonial_list, name="testimonial-list"),
    path("api/v1/testimonials/", testimonial_list, name="testimonial-list-v1"),
    path("api/testimonials/create/", testimonial_create, name="testimonial-create"),
    path("api/v1/testimonials/create/", testimonial_create, name="testimonial-create-v1"),
    path("api/testimonials/<int:pk>/", testimonial_detail, name="testimonial-detail"),
    path("api/v1/testimonials/<int:pk>/", testimonial_detail, name="testimonial-detail-v1"),
    path("api/core-values/", core_value_list, name="core-value-list"),
    path("api/core-values/create/", core_value_create, name="core-value-create"),
    path("api/core-values/<int:pk>/", core_value_detail, name="core-value-detail"),
    path("api/site-texts/", sitetext_list, name="site-text-list"),
    path("api/site-texts/create/", sitetext_create, name="site-text-create"),
    path("api/site-texts/<int:pk>/", sitetext_detail, name="site-text-detail"),
    path("api/site-images/", siteimage_list, name="site-image-list"),
    path("api/site-images/create/", siteimage_create, name="site-image-create"),
    path("api/site-images/<int:pk>/", siteimage_detail, name="site-image-detail"),
    path("api/process-steps/", process_step_list, name="process-step-list"),
    path("api/process-steps/create/", process_step_create, name="process-step-create"),
    path("api/process-steps/<int:pk>/", process_step_detail, name="process-step-detail"),
    path("api/products/", product_list, name="product-list"),
    path("api/products/create/", product_create, name="product-create"),
    path("api/products/<int:pk>/", product_detail, name="product-detail"),
    path("api/orders/", order_list, name="order-list"),
    path("api/orders/payments/", order_payments, name="order-payments"),
    path("api/orders/<int:pk>/", order_detail, name="order-detail"),
    path("api/blog/", blog_list, name="blog-list"),
    path("api/blog/create/", blog_create, name="blog-create"),
    path("api/blog/<int:pk>/", blog_detail, name="blog-detail"),
    path("api/settings/", site_settings_detail, name="settings-detail"),
    path("api/upload/<str:target>/<int:pk>/", upload_image, name="upload-image"),
    path("api/admins/", admin_user_list, name="admin-list"),
    path("api/admins/create/", admin_user_create, name="admin-create"),
    path("api/admins/<int:pk>/", admin_user_detail, name="admin-detail"),
    path("api/export/<str:model_name>/", export_csv, name="export-csv"),
    path("api/bulk-delete/<str:section>/", bulk_delete_section, name="bulk-delete"),
    path("api/account/password/", account_change_password, name="account-password"),
]
# API v2 (DRF) retirée : elle n'était appelée par aucun front, divergeait de l'API
# v1 et doublait la surface à sécuriser. Tout le dashboard consomme l'API v1 JSON.
